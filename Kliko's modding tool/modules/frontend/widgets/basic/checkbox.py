...
class CheckBox(LocalizedCTkCheckBox):
    ...
    def __init__(self, master, key: Optional[str] = None, modification: Optional[Callable[[str], str]] = None, **kwargs) -> None:
        ...
        if "checkbox_width" not in kwargs: kwargs["checkbox_width"] = 20
        if "checkbox_height" not in kwargs: kwargs["checkbox_height"] = 20
        if "checkmark_color" not in kwargs: kwargs["checkmark_color"] = ("#FFFFFF", "#1A1A1A")
